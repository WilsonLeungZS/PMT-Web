      ...
      <el-form ref="form" :model="taskLv2Form" label-width="140px" label-position="left" class="tl-edit-form" :rules="taskLv2FormRules">
        <el-tabs v-model="activeTabForLv2" type="card" ref="taskLv2Tabs" @tab-click="((tab, event)=>{changeTab(tab, event, 'taskLv2Form', 'activeTabForLv2')})">
         ...
         <el-tab-pane label="Status Tracing" name="tab_status_tracing">
          ...
          <!-- Second Tab -->
          <el-tab-pane v-if="false" label="Sub-Tasks List" name="tab_subtasks_list">
            <el-row>
              <el-col :span="24">
        ...
        <el-tabs v-model="activeTabForLv3" type="card" ref="taskLv3Tabs" @tab-click="((tab, event)=>{changeTab(tab, event, 'taskLv3Form', 'activeTabForLv3')})">
         ...
         <el-tab-pane label="Status Tracing" name="tab_status_tracing">
            ...
            <el-row>
              ...
              <el-col :span="11" :offset="1" v-if="lv3TaskItemRule.showRespLeader&&lv3TaskItemRule.showDeliverableTag">
                <el-form-item label="Assignee">
          ...
          <!-- Second Tab -->
          <el-tab-pane v-if="taskLv3Form.task_TypeTag!='Regular Task'&&lv3TaskItemRule.showSubTaskList&&false" label="Sub-Tasks List" name="tab_subtasks_list">
            <el-row>
              <el-col :span="24">
          ...
          <!-- Regular Task List -->
          <el-tab-pane v-if="taskLv3Form.task_TypeTag==='Regular Task'&&lv3TaskItemRule.showRegularTaskList&&false" label="Sub-Tasks List" name="tab_regular_list">
            <el-row>
              <el-col :span="24">
      ...
      <el-form ref="form" :model="taskLv4Form" label-width="140px" label-position="left" class="tl-edit-form" :rules="taskLv4FormRules">
        <el-tabs v-model="activeTabForLv4" type="card" ref="taskLv4Tabs" @tab-click="((tab, event)=>{changeTab(tab, event, 'taskLv4Form', 'activeTabForLv4')})">
          <el-tab-pane label="Basic Information" name="tab_basic_info">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item  v-if="lv3TaskItemRule.showDeliverableTag" label="Time Group">
                  <el-select  v-model="taskLv4Form.task_group_id" style="width: 100%" placeholder="Select Task Group...">
                    <el-option label="" value=""></el-option>
                    <el-option v-for="(taskgroup, index) in taskGroups" :key="index" :label="taskgroup.group_name" :value="taskgroup.group_id"></el-option>
                  </el-select>
                </el-form-item>
         ...
         <el-tab-pane label="Status Tracing" name="tab_status_tracing">
            ...
            <el-row>
              ...
              <el-col :span="11" :offset="1" v-if="lv4TaskItemRule.showDeliverableTag">
                <el-form-item label="Assignee">
  ...
  data () {
    return {
      ...
      taskLv3FormRules: {
        ...
        RegularTime: [{required: true, message: 'Could not be empty', trigger: 'blur'}],
      },
      lv4TaskItemRule: {
        disableParentNameInput: true,
        disableTaskEst: false,
  ...
  methods: {
    ...
    changeDaily () {
      if(this.$data.DailyOps === 1){
        ...
        this.$data.taskLv3Form.task_scheduletime = "Every "+this.$data.Scheduletime +" days"
        this.$data.taskLv4Form.task_scheduletime = "Every "+this.$data.Scheduletime +" days"   
      }
    },
    changeMonthly () {
...
        })
        ...
        if(iObj.task_status === 'Running' || iObj.task_status === 'Done'){
    ...
    async openTaskTab (iTaskName, Index, iPage, iSize,iShowRegular) {
      ...
      this.ChangeRegularStatus(iShowRegular)
      if (needGetTask !== -1) {
    ...
    async createTaskInPlanMode (iSubTaskLevel, iTaskObj) {
      ...
      if (Number(iSubTaskLevel) === 4) {
        ...
        // Set data default value
        const res  = await http.post('/tasks/getTaskByName',{
          reqTaskName : iTaskObj.task_name
        })
    ...
    async refreshLv2Task (iTaskId, iTaskName, iTaskIndex,iShowRegular) {
      console.log('Start to refresh level 2 task')
      this.openTaskTab(iTaskName, iTaskIndex, 1, 20,iShowRegular)
      var reqTaskGroupId = this.$data.currentTaskGroupId
      var reqTaskGroupFlag = this.$data.currentTaskGroupFlag
      ...
      if (res.data.status === 0) {
        ...
        if (rtnTask.task_level === 4) {
          ...
          if(res.data.data.task_TypeTag === 'Regular Task'){
            ...
            this.$data.taskLv4Form.task_deliverableTag = false
            const res1 = await http.post('/schedules/getSchedulesByTaskName',{
              reqTaskName : rtnTask.task_name
            })
            if(res1.data.status === 0){
    ...
    async saveLv4Task () {
      ...
      if (reqTask != null) {
        ...
        this.openTaskById(res.data.data.Id)
        this.refreshLv2Task(this.$data.refreshTaskId, this.$data.refreshTaskName, this.$data.refreshTaskIndex,this.$data.isRegular)
      }
    },
